...
const GooeyNav = ({
  ...
  const activateItem = (liEl, index, playParticles = true) => {
    ...
    if (playParticles && filterRef.current) {
  ...
  // Sync with scroll-based active section (controlled)
  useEffect(() => {
    ...
    if (controlledIndex === activeIndex) return;
    const liEl = navRef.current?.querySelectorAll('li')[controlledIndex];
    if (liEl) activateItem(liEl, controlledIndex, true);
  }, [controlledIndex]);
